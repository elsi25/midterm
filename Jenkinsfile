...
//execution and that each stage directive must specify its own agent section.s
    agent none
    stages {
        stage('Build') {
            agent {
        ...
        stage('Test') {
            ...
            steps {
                //This sh step executes pytest’s py.test command on testprograms/test_sample4.py, which runs a set of
        ...
        stage('Deliver') {
                    ...
                    steps {
                        ...
                        dir(path: env.BUILD_ID) {
                            ...
                            //This bundles your sample2.py Python application into a single standalone executable file
                            //and outputs this file to the dist workspace directory (within the Jenkins home directory).
                            sh "docker run --rm -v ${VOLUME} ${IMAGE} 'pyinstaller -F sample4.py'"
                        }
                    }
                    post {
                        success {
